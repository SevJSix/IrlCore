    ...
    public void onPlace(BlockPlaceEvent event) {
        if (event.getBlockPlaced().getType() == org.bukkit.Material.BEDROCK) {
            ...
            if (entityPlayer.inventory.getItemInHand() != null && entityPlayer.inventory.getItemInHand().hasTag() && entityPlayer.inventory.getItemInHand().getTag().hasKey("TowerBuilder")) {
                ...
                chicken.setPosition(position[0].getX() + 0.5, position[0].getY() + 2.0, position[0].getZ() + 0.5);
                entityPlayer.setPosition(position[0].getX() + 0.5, position[0].getY() + 2.0, position[0].getZ() + 0.5);
                    ...
                    public void run() {
                        ...
                        if (blocksPlaced >= 20) {
                            ...
                            Utils.run(() -> {
                                ...
                                BlockPosition p = world.getHighestBlockYAt(position[0]);
                                chicken.die();
                                entityPlayer.getBukkitEntity().teleport(location.clone().add(0, 1, 0));
                            });
                        }
                        entityPlayer.setPositionRotation(location.getX(), location.getY() + 2, location.getZ(), location.getYaw(), location.getPitch());
                        chicken.setPosition(location.getX(), location.getY() + 2, location.getZ() + 0.5);
